...
A simpler version of the language (i.e. no strand communication, no field lifting)
was described in a [PLDI 2012 paper](http://people.cs.uchicago.edu/~glk/pubs/#PLDI-2012).
New example programs are being added here to help you learn how to use Diderot,
...
You can create executable `foo` from `foo.diderot` with

	diderotc --exec foo.diderot
where the `diderotc` compiler is generated by the installation instructions below
(all of the examples assume you've followed these instructions).
Some examples benefit from different compilation options, as noted.
All of the examples have an introductory README.md (generated from the first
...
later examples assumes you've read earlier examples.  Enjoy!
